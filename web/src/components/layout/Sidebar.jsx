  ...
  return (
    <aside className="flex h-screen w-64 flex-col bg-gray-900">
      ...
      <div className="flex h-16 items-center gap-3 px-5">
        ...
        <div>
          <span className="text-sm font-semibold text-white">Legal Intel</span>
          <span className="ml-2 rounded bg-white/10 px-1.5 py-0.5 text-[10px] font-medium text-gray-400">v1.0</span>
        </div>
      </div>

      {/* Navigation */}
      <nav className="mt-2 flex-1 space-y-1 px-3 overflow-y-auto">
        <div className="mb-3 px-3 text-[10px] font-semibold uppercase tracking-wider text-gray-500">
          Menu
        </div>
        {navItems.map((item) => (
          <NavLink
      ...
      <div className="border-t border-white/10 p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <Badge color="brand" className="mt-0.5 text-[10px]">{user?.role || 'user'}</Badge>
          </div>
          <button
            onClick={logout}
